...
def create_pool(fee, tick_spacing, first_token, second_token, factory, deployer_acc):
    mock_time_pool_deployer = project.MockTimeUniswapV3PoolDeployer.deploy(
        sender=deployer_acc
    )
    tx = mock_time_pool_deployer.deploy(
        factory.address,
        first_token.address,
    ...
    return project.MockTimeUniswapV3Pool.at(tx.events[0].pool)
...
def check_tick_is_clear(pool, tick):
    ...
    assert liquidity_net == 0


def check_tick_is_not_clear(pool, tick):
    liquidity_gross = pool.ticks(tick)[0]
    assert liquidity_gross != 0


# Checks if one event in events data that has ContractLog instances for specific event data
def check_raised_event(events, *args):
    for event in events:
        if isinstance(event, ContractLog):
            if event.name == args[0]:
                for i in range(1, len(args)):
                    if event.args[i - 1] != args[i]:
                        break
                else:
                    return
    assert False
...
class PoolHelper:
    def __init__(
        self,
        token0: project.TestERC20,
        token1: project.TestERC20,
        factory: project.UniswapV3Factory,
        pool: project.MockTimeUniswapV3Pool,
        tick_spacing,
        swap_target: project.TestUniswapV3Callee,
    ):
        self.token0 = token0
        self.token1 = token1
    ...
    def swap(
        self, input_token, amount_in, amount_out, to, sender, sqrt_price_limit_x96=None
    ):
        ...
        input_token.approve(self.swap_target.address, MAX_UINT_256, sender=sender)
    ...
    def mint(self, recipient, tick_lower, tick_upper, liquidity, sender):
        self.token0.approve(self.swap_target.address, MAX_UINT_256, sender=sender)
        self.token1.approve(self.swap_target.address, MAX_UINT_256, sender=sender)
    ...
    def swap_exact_0_for_1(self, amount, to, sender, sqrt_price_limit_x96=None):
        return self.swap(self.token0, amount, 0, to, sender, sqrt_price_limit_x96)

    def swap_0_for_exact_1(self, amount, to, sender, sqrt_price_limit_x96=None):
        return self.swap(self.token0, 0, amount, to, sender, sqrt_price_limit_x96)

    def swap_exact_1_for_0(self, amount, to, sender, sqrt_price_limit_x96=None):
        return self.swap(self.token1, amount, 0, to, sender, sqrt_price_limit_x96)

    def swap_1_for_exact_0(self, amount, to, sender, sqrt_price_limit_x96=None):
        return self.swap(self.token1, 0, amount, to, sender, sqrt_price_limit_x96)
