import brownie
import pytest
from brownie import SqrtPriceMathTest, accounts

import utils
from brownie_tests.snapshots import match_snapshot
from brownie_utils import brownie_reverts_fix
from utils import MAX_UINT_128, MAX_UINT_256


@pytest.fixture(scope="module")
def sqrt_price_math():
    return SqrtPriceMathTest.deploy({"from": accounts[0]})
...
class TestGetNextSqrtPriceFromInput:
    ...
    def test_input_max_inputs(self, sqrt_price_math):
        ...
        max_amount_no_overflow = MAX_UINT_256 - int(
            (liquidity << 96) / sqrt_p - 1
        )
        assert (
            sqrt_price_math.getNextSqrtPriceFromInput(
